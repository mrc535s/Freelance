<?php
	...
	*/
	...
	function print_page_header( $header, $caption = '' ){
		...
		if( !empty($caption) ){
			echo '<span class="gdl-header-caption">' . $caption . '</span>';
		}
		echo '</div>';	
	}
	
	// Print header of each item
	function print_item_header( $header, $divider = false, $icon = false ){
		if(!empty($header)){
			echo '<div class="gdl-item-header-wrapper">';
			if( !empty($icon) ){
				$thumbnail = wp_get_attachment_image_src( $icon , 'full' );
				$alt_text = get_post_meta($icon , '_wp_attachment_image_alt', true);
				echo '<img src="' . $thumbnail[0] . '" alt="' . $alt_text . '" />';
			}
			echo '<h3 class="gdl-item-header-title">' . $header . '</h3>';
			
			if( $divider ){
				echo '<div class="gdl-item-header-bottom-bar"></div>';
			}
			echo '</div>'; // gdl-item-header-wrapper
		}
	}
	...
	// Print column 
	function print_blog_port_description($item_xml){
		echo '<div class="blog-port-desc-title">';
		print_item_header( find_xml_value($item_xml, 'header'), true, find_xml_value($item_xml, 'header-icon') );
		echo '</div>';
		...
		if( !empty( $button_link ) ){
			echo '<a href="' . $button_link . '" class="gdl-column-read-more">';
			echo find_xml_value($item_xml, 'button-text');
			echo '</a>';
		}
	}
	...
	function print_column_service($item_xml){
		...
		switch(find_xml_value($item_xml,'title')){
		
			case 'Cloud Services':
			echo '<a href="services/cloud">';
			break;
	...
	function print_divider($item_xml){
		echo '<div class="clear"></div>';
		echo '<div class="divider-scroll-top">' . find_xml_value($item_xml, 'text') . '</div>';
	...
	function print_stunning_text($item_xml){
		...
		$button_link =  find_xml_value($item_xml, 'button1-link');
		if( !empty( $button_link ) ){
			$button_title =  find_xml_value($item_xml, 'button1-title');
			echo '<a href="' . $button_link . '" class="stunning-text-button">' . $button_title . '</a>';
		}

		$button_link =  find_xml_value($item_xml, 'button2-link');
		if( !empty( $button_link ) ){
			$button_title =  find_xml_value($item_xml, 'button2-title');
			echo '<a href="' . $button_link . '" class="stunning-text-button">' . $button_title . '</a>';
		}
