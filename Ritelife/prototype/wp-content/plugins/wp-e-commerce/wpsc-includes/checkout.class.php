...
function wpsc_have_valid_shipping_zipcode(){
	if(empty($_SESSION['wpsc_zipcode']) || ('Enter Postcode' == $_SESSION['wpsc_zipcode']) && ($_SESSION['wpsc_update_location']))
		return true;
	else
		return false;
...
function wpsc_shipping_country_list( $shippingdetails = false ) {
	...
	if ( ($zipvalue != '') && ($zipvalue != 'Enter Postcode') ) {
		$color = '#000';
	} else {
		$zipvalue = 'Enter Postcode';
		$color = '#999';
	}
	...
	if ( $uses_zipcode == true ) {
		$output .= " <input type='text' style='color:" . $color . ";' onclick='if (this.value==\"Enter Postcode\") {this.value=\"\";this.style.color=\"#000\";}' onblur='if (this.value==\"\") {this.style.color=\"#999\"; this.value=\"Enter Postcode\"; }' value='" . $zipvalue . "' size='16' name='zipcode' id='zipcode'>";
	}
	return $output;
}
