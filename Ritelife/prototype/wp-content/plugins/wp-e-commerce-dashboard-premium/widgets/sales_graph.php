...
if ( ! class_exists ( 'ses_wpscd_sales_graph' ) ) {

	class ses_wpscd_sales_graph {
		...
		// Working Variables
		private $completed_sales = Array();
		private $pending_sales = Array();
		...
		function __construct() {
			...
			switch ( $this->settings['ses_wpscd_sales_graph_period'] ) {
			...
			$this->completed_sales = ses_wpscd_date_range_array ( $this->from_unixtime, $this->to_unixtime, $this->settings['ses_wpscd_sales_graph_detail'] );
			$this->pending_sales = ses_wpscd_date_range_array ( $this->from_unixtime, $this->to_unixtime, $this->settings['ses_wpscd_sales_graph_detail'] );
		...
		function prepare() {
			...
			if ( isset ( $this->settings['ses_wpscd_sales_graph_detail'] ) && $this->settings['ses_wpscd_sales_graph_detail'] != '1' ) {

				$period = $this->settings['ses_wpscd_sales_graph_detail'];
			...
			foreach ( $results as $row ) {
				...
				if ($row['status'] == 'Complete') {
					$this->completed_sales["$idx"] += $row['num_items'];
				} else {
					$this->pending_sales["$idx"] += $row['num_items'];
				}
		...
		function render() {
			...
			?>
			<div id="ses-wpscd-sales-graph" style="width:100%; height:<?php echo $this->height; ?>px; position:relative;"></div>
	
			<script type="text/javascript">
	
				var sales_completed = [ <?php echo $this->output_data_array ( &$this->completed_sales ); ?> ];
				var sales_pending = [ <?php echo $this->output_data_array ( &$this->pending_sales ); ?> ];
				...
				jQuery(function(){
					...
					var plot = jQuery.plot(jQuery("#ses-wpscd-sales-graph"), [ { label: "<?php _e('Completed sales', 'ses_wpscd'); ?>", data: sales_completed }
					<?php 
						if ( ! empty ( $this->settings['ses_wpscd_sales_graph_show_pending'] ) ) {
							echo ', { label: "'.__('Pending sales', 'ses_wpscd').'", data: sales_pending }';
						}  ?>
						], {
						series: {
							lines: { show: true },
				    ...
				    jQuery("#ses-wpscd-sales-graph").bind("plothover", function (event, pos, item) {
			            if (item) {
		...
		function config() {
			...
			if ( ! empty ( $_REQUEST['ses_wpscd_sales_graph_period'] ) ) {
		
				if ( wp_verify_nonce ( $_REQUEST['_wpnonce'], 'ses_wpscd_sales_graph') ) {
		
					$this->settings['ses_wpscd_sales_graph_period'] = $_REQUEST['ses_wpscd_sales_graph_period'];
					$this->settings['ses_wpscd_sales_graph_detail'] = $_REQUEST['ses_wpscd_sales_graph_detail'];
					$this->settings['ses_wpscd_sales_graph_show_pending'] = $_REQUEST['ses_wpscd_sales_graph_show_pending'];
			...
			echo wp_nonce_field('ses_wpscd_sales_graph');
			?>
			<?php
				$checked = Array();
				$checked['7'] = '';
				...
				$checked['270'] = '';
				$checked['365'] = '';
				if ( isset ( $this->settings['ses_wpscd_sales_graph_period'] ) ) {
					$checked[$this->settings['ses_wpscd_sales_graph_period']] = ' selected';
				} else {
					$checked['30'] = ' selected';
				}
			...
			?>
			<p>
			<label for="ses_wpscd_sales_graph_period"><?php _e('Show: ', 'ses_wpscd'); ?></label><br/>
			<select name="ses_wpscd_sales_graph_period">
			...
			<?php
				...
				$checked['21'] = '';
				$checked['28'] = '';
				if ( isset ( $this->settings['ses_wpscd_sales_graph_detail'] ) ) {
					$checked[$this->settings['ses_wpscd_sales_graph_detail']] = ' selected';
				} else {
					$checked['1'] = ' selected';
				}
			?>
			<p>
			<label for="ses_wpscd_sales_graph_period"><?php _e('Group results by: ', 'ses_wpscd'); ?></label><br/>
			<select name="ses_wpscd_sales_graph_detail">
			...
			<p>
			<input <?php if ( ! empty ( $this->settings['ses_wpscd_sales_graph_show_pending'] ) ) echo "checked='checked'"; ?> type="checkbox" name="ses_wpscd_sales_graph_show_pending">
			<label for="ses_wpscd_sales_graph_show_pending"><?php _e('Show Pending Sales: ', 'ses_wpscd'); ?></label><br/>
			</p>
			<?php
		}
